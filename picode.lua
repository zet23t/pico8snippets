...
function test(code,expected_values,init_values)
	...
	local co = cocreate(f)
	local suc,errstr = coresume(co)
	local err
	if not suc and errstr then
		-- printh("execution error: "..errstr)
		printh(trace(co,errstr))
		--dump(code_addr)
		err = true
	end
	for k,v in pairs(expected_values) do
...
-- ("r = x(y(2),1)", {r = 3},{y=function(a) return a,a end, x=function(a,b) return a+assert(b) end})
...
-- ('local function fun() a = 0 end fun()', {a = 0,fun = 0}, {fun=0})
-- test("local function fun(x) a = x end fun(1)", {a = 1})
-- test("local function fun(x) local function f2(x) a = x end f2(x) end fun(1)", {a = 1})
-- test("local function fun(x) local function f2() a = x end f2(x) end fun(1)", {a = 1})
